...
{	
	...
	private function import_item($item)
	{
		...
		if(isset($item['name'])) {
			...
			$product['meta_title'] = trim($item['name']);
		}
	 			...
	 			foreach($images as $image)
	 			{
	 				...
	 				if(!empty($image))
	 				{
						...
						if(!$this->db->result('filename'))
						{
							...
							if($new_image_name = $this->download_image($product_id, $image, $this->translit($product['name']).'-'.$index)){
								$this->products->add_image($product_id, $new_image_name);
							}
						}
					}
					$index++;
	...
	private function download_image($product_id, $img_url, $image_filename)
	{
		static $mime2ext = [
			'image/gif' =>'gif',
			'image/jpeg' =>'jpeg',
		...
		$inputObjectStream = fopen('php://memory', 'w+b');
		fwrite($inputObjectStream, $res);
		unset($res);

		// Расширение оригинального файла
		$basename		= explode('&', pathinfo($img_url, PATHINFO_BASENAME));
		$uploaded_file	= array_shift($basename);
		$ext			= strtolower(pathinfo($uploaded_file, PATHINFO_EXTENSION));
			
		if( empty($ext) ){
				
			$mime_type = strtolower(mime_content_type($inputObjectStream));
			$ext = $mime2ext[$mime_type] ?: false;
			if (!$ext){
				fclose($inputObjectStream);
				return false;
			}
		}

		// Если такой файл существует, нужно придумать другое название
		$new_name	= urldecode($image_filename . '.' . $ext);
		$base		= pathinfo($new_name, PATHINFO_FILENAME);
		
		// Добавляем изображение только если такого еще нет в этом товаре
		$this->db->query('SELECT filename FROM __images WHERE product_id=? AND (filename=? OR filename=?) LIMIT 1', (int) $product_id, $image_filename, $new_name);
		...
		if(!$this->db->result('filename'))
		{
			...
			$outputObjectStream = fopen($this->config->root_dir.$this->config->original_images_dir.$new_name , 'w+b');
			rewind($inputObjectStream);
			stream_copy_to_stream($inputObjectStream, $outputObjectStream);
			fclose($outputObjectStream);
		}
		else{
			$new_name = false;
		}
